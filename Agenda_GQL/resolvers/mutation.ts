...
import { getInformationFromCountry } from "../lib/apifuntions.ts";
import { getInformationFromPhone } from "../lib/apifuntions.ts";
import { ContactModel,ContactModelType } from "../db/Contacts.ts";
...
export const Mutation = {
  addContact: async (_: unknown,args: { name: string; phone: string }): Promise<ContactModelType> => {
    try {
      ...
      if(!phoneInformation.is_valid) { throw new GraphQLError("Invalid phone number");}
      const countryInfo = await getInformationFromCountry(phoneInformation.country);
      if(countryInfo.length===0) {throw new GraphQLError("Invalid country");}
