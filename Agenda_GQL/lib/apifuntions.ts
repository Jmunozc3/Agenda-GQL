...
export const getInformationFromPhone = async (phone: string):Promise<PhoneInfo> => {
...
export const getInformationFromCountry = async (country: string):Promise<CountryInfo[]> => {
    const API_KEY = Deno.env.get("API_KEY");
    if (!API_KEY) {throw new Error("API_KEY not found");}
